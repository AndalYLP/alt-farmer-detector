...
@app_commands.command(name="added", description=FriendsDesc.added)
@app_commands.describe(target=FriendsDesc.target, usernames=FriendsDesc.usernamesAdded)
async def added_with(interaction: discord.Interaction, target: str, usernames: str):
    logger.log(
        "COMMAND",
    ...
    try:
        ...
        logger.exception(e)
        await interaction.response.send_message(embed=error_embed("An error occurred."))
