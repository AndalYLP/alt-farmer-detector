    ...
    async def List(interaction: discord.Interaction):
        ...
        async def NextPageCB(interaction):
            ...
            if page < len(pages) - 1:
                ...
                embed = discord.Embed(color=8585471,title="User list",description="".join(f"**{i+1+(page*15)}.** ``{str(v["Username"])}`` **|** {str(v["UserID"])} **|** **{str(v.get("GroupName", "None"))}**\n" for i,v in enumerate(pages[page])))
    ...
    async def bygroup(interaction: discord.Interaction, groupname):
        ...
        async def NextPageCB(interaction):
            ...
            if page < len(pages) - 1:
                ...
                embed = discord.Embed(color=8585471,title=f"{groupname} Group list",description="".join(f"**{i+1+(page*15)}.** ``{str(v["Username"])}`` **|** {str(v["UserID"])} **|** **{str(v.get("GroupName", "None"))}**\n" for i,v in enumerate(pages[page])))
                await interaction.response.edit_message(embed=embed, view=view)
            else:
                NextPage.disabled = True
