...
class BatchObject:
    ...
    def getByImageUrl(self, url: str) -> ThumbnailObject | None:
        result = [
            thumbnail for thumbnail in self.thumbnails if thumbnail.imageUrl == url
        ]
    ...
    def getByRequestId(self, requestId: str) -> ThumbnailObject | None:
        result = [
            ...
            if thumbnail.requestId == requestId
        ]
        return result[0] if result else None

    def getByTargetId(self, targetId: int) -> ThumbnailObject | None:
        result = [
            thumbnail for thumbnail in self.thumbnails if thumbnail.targetId == targetId
        ]
        return result[0] if result else None
    ...
    def getAllImageUrls(self) -> dict[any, str]:
        ...
        """
        return {
            thumbnail.requestId: thumbnail.imageUrl for thumbnail in self.thumbnails
        }
...
def getUsersAvatar(
    *userIds: int,
    ...
    response = requests.get(
        thumbnailsApi
        + f"/v1/users/avatar-{type}?userIds={",".join(map(str, userIds))}&size={size}&format={format}&isCircular={isCircular}",
...
def getUsersAvatarFromUsername(
    *usernames: int,
    type: str = "headshot",
    size: str = "48x48",
    format: str = "Png",
    isCircular: bool = False,
) -> tuple["BatchObject | ThumbnailObject", "UserGroup"]:
...
async def batch(*batchObjects: ThumbnailBatchObject) -> BatchObject:
        ...
        while True:
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    print(
                        f"Error in the request: {response.status}\n{await response.text()}\nTrying again in 500ms {f"({attempts}/3)" if errorCode != 0 else ""}",
                        file=sys.stderr,
                    )
    ...
    groups = [
        [batchObject for batchObject in batchObjects[i : i + 50]]
        for i in range(0, len(batchObjects), 50)
    ]
    ...
    async def proccessGroups():
        async with aiohttp.ClientSession() as session:
            ...
            responses = await asyncio.gather(
                *[runWithSemaphore(task) for task in tasks], return_exceptions=True
            )
