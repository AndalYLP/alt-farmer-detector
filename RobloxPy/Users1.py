...
class UserGroup:
    ...
    def getByRequestedUsername(self, requestedUsername: str) -> User | None:
        result = [
            user for user in self.users if user.requestedUsername == requestedUsername
        ]
        return result[0] if result else None
...
def getIds(*usernames: str, excludeBanned: bool = True) -> dict[str, int | None]:
    response = requests.post(
        userApi + "/v1/usernames/users",
        json={"usernames": list(usernames), "excludeBannedUsers": excludeBanned},
        headers={"Cookie": cookies.getCookie()},
    )
    ...
    if response.status_code == 200:
        ...
        if data and "id" in data[0]:
            ...
            result.update(
                {username: result.get(username, None) for username in usernames}
            )
        ...
        raise requests.exceptions.HTTPError(
            f"Error in the request with {userApi}'s Endpoint: {response.status_code}",
            response.text,
        )
...
def getUsernames(*userIds: int, excludeBanned: bool = True) -> dict[int, str, None]:
    response = requests.post(
        userApi + "/v1/users",
        json={"userIds": list(userIds), "excludeBannedUsers": excludeBanned},
        headers={"Cookie": cookies.getCookie()},
    )
    ...
    if response.status_code == 200:
        ...
        raise requests.exceptions.HTTPError(
            f"Error in the request: {response.status_code}\n{response.text}"
        )
