...
class ServerGroup:
    def __init__(self, data):
        ...
        self.servers = [Server(server) for server in self.data]

    async def getPlayerThumbnails(
        self,
        type: str = "AvatarHeadShot",
        size: str = "48x48",
        format: str = "png",
...
def getServerPage(
    ...
    response = requests.get(
        gamesApi
        + f"/v1/games/{gameId}/servers/{serverType}?sortOrder={sortOrder}&excludeFullGames={excludeFullGames}&limit={limit}&cursor={cursor}",
        headers={"Cookie": cookies.getCookie() if useCookie else ""},
    )
...
def getAllServers(
    ...
    currentPage = getServerPage(
        gameId, serverType, excludeFullGames=excludeFullGames, useCookie=useCookie
    )
