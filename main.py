...
async def UserStatus(userPresences, channel, AltChannel, todelete, todelete2):
    if userPresences:
        for doc in userPresences:
            ...
            embed = discord.Embed(color=color if LobbyStatus == "True"  and PresenceType == 2 else 1881856,title=title,description=description if PresenceType == 2 and not doc["rootPlaceId"] == None else None)
